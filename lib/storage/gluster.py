...
class GlusterVolume(object):
  ...
  def __init__(self, server_addr, port, volume, _run_cmd=utils.RunCmd):
    ...
    self._port = port
    self._volume = volume
    self.mount_point = io.PathJoin(
      pathutils.DEFAULT_SHARED_FILE_STORAGE_DIR,
      "%s:%d:%s" % (self.server_ip, self.port, self.volume)
    )
...
class GlusterStorage(base.BlockDev):
  ...
  def __init__(self, unique_id, children, size, params, dyn_params,
    ...
    self.path = path
    self.driver = driver
    self.dev_path = io.PathJoin(self.volume.mount_point, self.path)
    self.file = None # we might not be mounted yet
